...
// MaxTotal=2：允许一个后台预渲染 + 一个按需渲染并行，靠 pool 限流防内存翻倍。
...
// renderOnePage 实时渲染单页（打开文档只渲染一页），返回 JPEG 字节。
func renderOnePage(pdfPath string, page1 int, dpi int) ([]byte, error) {
	if err := initPdfium(); err != nil {
		return nil, err
	}
	instance, err := pdfiumPool.GetInstance(time.Second * 60)
	...
	pr, err := instance.RenderPageInDPI(&requests.RenderPageInDPI{
		DPI:  dpi,
		Page: requests.Page{ByIndex: &requests.PageByIndex{Document: doc.Document, Index: page1 - 1}},
	})
	if err != nil {
		return nil, err
	}
	...
	pr.Cleanup() // WASM 模式必须释放图像资源，否则内存不归还
...
func handlePageImage(w http.ResponseWriter, r *http.Request, u *User) {
	...
	data, err := renderOnePage(entry.Path, page+1, dpi)
	if err != nil {
